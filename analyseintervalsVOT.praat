...
form analyse intervals
	word subject sub7
	word vowelp 1
	sentence Sound_folder /Users/zhiyangao/Desktop/englishFiles/vot/
	sentence TextGrid_folder /Users/zhiyangao/Desktop/englishFiles/vot/
	word Analysis_tier AutoVOT
...
...'tab$'F1'tab$'F2'tab$'F3'tab$'wordtype'tab$'sp'tab$'tones'tab$'stim'tab$'MaxInt'tab$'f0max'tab$'f0min'tab$'f0range
...
#Get time values of beginning and end of each interval for treatment
		...
		for iInterval from 1 to nIntervals
			...
			label_end = Get end time of interval... tier iInterval
			label_phrase = Get interval at time... 3 label_end
			stim$ = Get label of interval... 3 label_phrase
...
#print out results
				...
				...'tab$''f2$''tab$''f3$''tab$''type$''tab$''senPosition$''tab$''tones$''tab$''stim$''tab$''max_int''tab$''f0max''tab$''f0min''tab$''f0range'
			endif
		endfor
