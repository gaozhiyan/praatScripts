...
form analyse intervals
	word subject sub7
	word vowelp 1
	sentence Sound_folder /Users/zhiyangao/Desktop/englishFiles/phrases/askher/
	sentence TextGrid_folder /Users/zhiyangao/Desktop/englishFiles/phrases/askher/
...
printline 'tab$'extract'tab$'label'tab$'subject'tab$'vp'tab$'duration'tab$'pitch'tab$'intensity
...'tab$'F1'tab$'F2'tab$'F3'tab$'f1mid'tab$'f2mid'tab$'f3mid'tab$'stim'tab$'MaxInt'tab$'f0max'tab$'f0min'tab$'f0range
...
#Get time values of beginning and end of each interval for treatment
		...
		for iInterval from 1 to nIntervals
			...
			#type$ = Get label of interval... 4 iInterval
			##senPosition$ = Get label of interval... 4 iInterval
			##tones$ = Get label of interval... 5 iInterval
			#stim$ = Get label of interval... 1 iInterval
			subject$ = subject$
			vp$ = vowelp$
...
#formants
				...
				f1_mean = Get value at time... 1 mid Hertz Linear
				call set_undefined f1
				f1$ = value$
				f2 = Get mean... 2 start end Hertz
				f2_mean = Get value at time... 2 mid Hertz Linear
				call set_undefined f2
				f2$ = value$
				f3 = Get mean... 3 start end Hertz
				f3_mean = Get value at time... 3 mid Hertz Linear
...
#print out results
				...
				...'tab$''f2$''tab$''f3$''tab$''f1_mean''tab$''f2_mean''tab$''f3_mean''tab$''stim$''tab$''max_int''tab$''f0max''tab$''f0min''tab$''f0range'
			endif
		endfor
