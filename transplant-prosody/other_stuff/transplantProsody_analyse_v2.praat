Read Table from comma-separated file: "auto_pitch2.csv"
num_combos = Get number of rows

textgrids_path$ = "/Users/tim/Dropbox/Perception_Study/Pilot/original"
output_path$ = "/Users/tim/Documents/sound_files/auto_output"

writeFileLine: "descriptive_data_v2.csv", "stimulus_name,speech_rate,pitch_mean,pitch_sd,FL_ratio,AL_ratio,combined_ratio"

for combo to num_combos
	selectObject: "Table auto_pitch2"
	sentence_label$ = Get value: combo, "sentence"
	combo_id$ = Get value: combo, "speaker_combo"
	if left$ (combo_id$, 1) != "L"
		receiver_id$ = "L2_" + left$ (combo_id$, 4)
		control_receiver = 2
	else
		receiver_id$ = left$ (combo_id$, 4)
		control_receiver = 1
	endif
	donor_id$ = right$ (combo_id$, 4)
	control_donor = 0
	if left$ (donor_id$, 1) != "L"
		donor_id$ = "L2_" + donor_id$
		if control_receiver != 1
			control_donor = 1
		endif
	endif
	rec_frame_length = Get value: combo, "r_frame"
	rec_minimum_pitch = Get value: combo, "r_min"
	rec_maximum_pitch = Get value: combo, "r_max"
	don_frame_length = Get value: combo, "d_frame"
	don_minimum_pitch = Get value: combo, "d_min"
	don_maximum_pitch = Get value: combo, "d_max"
	...
	for rec_spkr to n_rec_spkr
		...
		for don_spkr to n_don_spkr
			...
			for lp from 1 to num_loops
				if control_donor = 1 and (lp != 4)
					goto NEXT_LOOP
				endif
				if control_receiver = 1 and (lp != 8)
					goto NEXT_LOOP
				endif
				if control_receiver = 2 and (lp = 8)
					goto NEXT_LOOP
				endif
				...
				Rename: new_name$
				selectObject: "Sound " + receiver_name$
				plusObject: "PitchTier " + donor_name$
				plusObject: "Manipulation " + donor_name$
				plusObject: "PitchTier " + receiver_name$
				plusObject: "DurationTier " + receiver_name$
				plusObject: "Sound " + receiver_name$
				plusObject: "IntensityTier " + new_name$ + "_norm"
				plusObject: "IntensityTier " + new_name$
				plusObject: "Intensity " + new_name$
				plusObject: "Sound " + donor_name$
				Remove

				@writeDescriptiveData
				label NEXT_LOOP
			endfor
		endfor
	endfor
	selectObject: "Table " + sentence_label$ + "_L1"
	plusObject: "Table " + sentence_label$ + "_var_words"
	plusObject: "Table " + sentence_label$ + "_pauses"
	plusObject: "Table " + sentence_label$ + "_var_pauses"
	plusObject: "Table " + sentence_label$ + "_pauses_" + receiver_id$
	plusObject: "Table " + sentence_label$ + "_pauses_" + donor_id$
	Remove
endfor

procedure writeDescriptiveData
	@getSpeechRate
	@getPitch
	@getRhythm
	if control_receiver = 1
		.write_name$ = receiver_name$
	else
		.write_name$ = new_name$
	endif
	appendFileLine: "descriptive_data_v2.csv", .write_name$, ",", getSpeechRate.rate, ",", getPitch.mean_erb, ",", getPitch.sd_erb, ",", getRhythm.fl_ratio, ",", getRhythm.al_ratio, ",", getRhythm.comb_ratio
endproc

procedure getSpeechRate
	selectObject: "TextGrid " + new_name$
	.length = Get total duration
	.num_syl = Count intervals where: 5, "is not equal to", ""
	.num_syl_check = Count intervals where: 4, "is not equal to", ""
	if .num_syl != .num_syl_check
		appendInfoLine: "Check syllables in TextGrid ", new_name$
	endif
	.rate = .num_syl / .length
endproc
...
procedure getPitch
	...
	Extract pitch tier
	.sd_hz = Get standard deviation (points): start_sound, end_sound
	.sd_erb = 16.7 * log10(1 + .sd_hz / 165.4)
	.mean_hz = Get mean (points): start_sound, end_sound
	.mean_erb = 16.7 * log10(1 + .mean_hz / 165.4)
	Remove
endproc
...
procedure getRhythm
	...
	.num_comb = 0
	.num_no_comb = 0
	.tot_syl = Get number of intervals: 5
	for .syl to .tot_syl
		.fl_syl_lab$ = Get label of interval: 5, .syl
		.start_fl = Get starting point: 5, .syl
		.end_fl = Get end point: 5, .syl
		.fl_dur = .end_fl - .start_fl
		if .fl_syl_lab$ = "2" or .fl_syl_lab$ = "3"
			.num_fl += 1
			.cum_fl += .fl_dur
		elsif .fl_syl_lab$ = "0"
			.num_no_fl += 1
		...
		.al_syl_lab$ = Get label of interval: 6, .syl
		.start_al = Get starting point: 6, .syl
		.end_al = Get end point: 6, .syl
		.al_dur = .end_al - .start_al
		if .al_syl_lab$ = "2" or .al_syl_lab$ = "3"
			.num_al += 1
			.cum_al += .al_dur
		elsif .al_syl_lab$ = "0"
			.num_no_al += 1
		...
		if .fl_syl_lab$ = "2" or .fl_syl_lab$ = "3" or .al_syl_lab$ = "2" or .al_syl_lab$ = "3"
			.num_comb += 1
			.cum_comb += .fl_dur
		elsif .fl_syl_lab$ = "0" and .al_syl_lab$ = "0"
			.num_no_comb += 1
			.cum_no_comb += .fl_dur
		endif
	endfor
	.mean_fl = .cum_fl / .num_fl
	.mean_no_fl = .cum_no_fl / .num_no_fl
	.fl_ratio = .mean_fl / .mean_no_fl
	.mean_al = .cum_al / .num_al
	.mean_no_al = .cum_no_al / .num_no_al
	.al_ratio = .mean_al / .mean_no_al
	.mean_comb = .cum_comb / .num_comb
	.mean_no_comb = .cum_no_comb / .num_no_comb
	.comb_ratio = .mean_comb / .mean_no_comb
endproc
